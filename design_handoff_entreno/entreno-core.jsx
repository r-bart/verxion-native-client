...
function buildEntreno(t) {
  ...
  const routine = {
    ...
    planned: 36,
    weekFrac: fresh ? 1 / 7 : 3 / 7,     // avance dentro de la semana en curso (día 3 de 7)
  };
  ...
  // sesiones — historial reciente (fuente única: sesiones-data)
  const sessions = (typeof VX_SESSIONS !== "undefined") ? VX_SESSIONS : [];
...
function ScoreChip({ state }) {
  const word = state === "ahead" ? "Vas adelantado" : state === "on" ? "En objetivo" : "Vas justo";
  return (
...
function WeekBlock({ routine, viz }) {
  ...
  const wf = (typeof routine.weekFrac === "number" && routine.weekFrac > 0 && routine.weekFrac < 1) ? routine.weekFrac : null;

  if (viz === "ring") {
    const size = 78, sw = 8, r = (size - sw) / 2, c = size / 2, frac = Math.min(1, (week - 1 + (routine.weekFrac || 0)) / weeks);
    ...
    return (
      <div className="e-week e-week--ring">
        ...
        <div className="e-week__side">
          ...
          <div className="e-week__score"><ScoreChip state={scoreState} /></div>
        </div>
      </div>
    );
  }
    ...
    <div className="e-week">
      ...
      <div className="e-week__cells">
        {Array.from({ length: weeks }).map((_, i) => {
          ...
          return (
            <span key={i} className={"e-week__cell " + cls}>
              {cls === "is-now" && wf != null && <span className="e-week__fill" style={{ width: Math.max(9, wf * 100) + "%" }} />}
            </span>
          );
        })}
      </div>
...
// ── routine hero (the centerpiece) — abre el detalle de la rutina ───────────
function RoutineHero({ model }) {
  const { routine, weekViz } = model;
  return (
    <button className="e-hero e-hero--tap" onClick={() => vxNav(VX.RUTINA, { rutina: "ppl-hipertrofia" })}>
      <div className="e-hero__top">
        <div className="e-eyebrow">RUTINA ACTIVA</div>
        <Icon.chevronRight size={15} style={{ color: "rgba(255,255,255,0.3)" }} />
      </div>
...
// ── enlace "todas las rutinas" ───────────────────────────────────────────────
function AllRoutinesLink() {
  const n = (typeof RD_ROUTINES !== "undefined") ? RD_ROUTINES.length : 5;
  return (
    <button className="e-allrt" onClick={() => vxNav(VX.RUTINAS)}>
      <span className="e-allrt__l"><EIcon.rows size={15} /> Todas tus rutinas</span>
      <span className="e-allrt__r">{n} <Icon.chevronRight size={15} /></span>
    </button>
  );
}
    ...
    <div className="e-spine">
      {model.spine.map((d, i) => {
        ...
        const onTap = d.status === "live"
          ...
          : !isRest
          ? () => vxNav(VX.DIA, { rutina: "ppl-hipertrofia", dia: i, hoy: false })
          : null;
        return (
...
function EmptyInvite() {
  return (
    <div className="e-invite">
      ...
      <div className="e-prompt" onClick={() => vxNav(VX.AGENTE, { caso: "inicio" })} style={{ cursor: "pointer" }}>
...
function SessionsView({ model }) {
  ...
  const avgVol = sessions.length ? sessions.reduce((a, s) => a + (s.volT || 0), 0) / sessions.length : 0;
  const blockVol = Math.round(avgVol * (routine.done || sessions.length)) || "—";
  if (empty) {
    return (
  ...
  return (
    <>
      <div className="e-sumrow">
        ...
        <div className="e-sum"><b>{blockVol}<em>t</em></b><span>volumen · bloque</span></div>
        <div className="e-sum"><b>+8<em>%</em></b><span>vs semana 2</span></div>
      </div>
      <div className="e-seshead">
        <span className="e-section" style={{ padding: 0 }}>Historial reciente</span>
        <button className="e-seslink" onClick={() => vxNav(VX.SESIONES)}>Ver todo <Icon.chevronRight size={13} /></button>
      </div>
      <div className="e-sessions">
        {sessions.map((s, i) => {
          const cfg = DAY_TYPE[s.type];
          return (
            <div className="e-sess e-sess--tap" key={i} onClick={() => vxNav(VX.SESION_DET, { sesion: s.slug })} style={{ cursor: "pointer" }}>
              ...
              <div className="e-sess__meta">
                <div className="e-sess__vol">{(s.volT != null ? s.volT.toFixed(1).replace(".", ",") : eInt(s.vol))} <em>{s.volT != null ? "t" : "kg"}</em></div>
      ...
      <div className={"e-scroll" + (showBanner ? " e-scroll--banner" : "")}>
        <div className="e-head">
          <div className="e-headrow">
            ...
            <button className="e-libbtn" aria-label="Todas las rutinas" onClick={() => vxNav(VX.RUTINAS)}><EIcon.layers size={20} /></button>
          </div>
...
Object.assign(window, {
  buildEntreno, EIcon, EBubble, ScoreChip, WeekBlock, RoutineHero, AllRoutinesLink,
