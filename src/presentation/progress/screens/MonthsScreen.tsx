...
export function MonthsScreen() {
  const { data, isLoading, isError, refetch } = useMonths();
  ...
  if (!data || data.length === 0) {
    return (
      <SafeAreaView className="flex-1" style={bg}>
        <ScreenSelector options={historyScreens} />
        <EmptyState icon="📈" title="No monthly data" message="Start training to see your monthly summaries." />
      </SafeAreaView>
    );
  }
    ...
    <SafeAreaView className="flex-1" style={bg}>
      ...
      >
        {data.map((month) => (
          <Pressable
            key={month.month}
            onPress={() => router.push(`/progress/months/${month.month}` as any)}
